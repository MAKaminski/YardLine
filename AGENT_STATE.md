...
- ~~Redirect URL unset~~ RESOLVED: allowlisted and confirmed working in the auth logs.
- **EMAIL RATE LIMIT: 2/hour, project-wide.** Custom SMTP via Resend was configured and rolled back — GoTrue returned `535 "Authentication credentials invalid"` on every send, which also broke Lace Luxx logins on the shared project. Rolled back to the built-in sender at 02:54Z; Supabase auto-reset the limiter from 30 back to `2/1h`. Resend side is proven good (verified domain, delivered test send), so the likely cause is the SMTP username needing to be the literal string `resend`. Untested. FIX + workaround (log reps in the night before) at the top of HANDOFF.md.
